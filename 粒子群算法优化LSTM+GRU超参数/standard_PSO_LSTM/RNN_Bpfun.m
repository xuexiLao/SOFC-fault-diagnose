function err = RNN_Bpfun(x, P_train, T_train, P_test, T_test,N,T)
%% 训练与测试LSTM网络
%% 输入
% x:一个个体的初始权值和阈值
% P_train:训练样本输入
...
if isempty(recall)                                         %判断数据是否为空（未赋值）,为空则是ture,否则为false
  precision = zeros(4, N*(T+1));                                %精确率/查准率
  recall = zeros(4, N*(T+1));                                   %召回率/查全率
  f1_score = zeros(4, N*(T+1));                                 %F1-Score
  macro_f1=zeros(1,N*(T+1));
  accurate=zeros(1,N*(T+1));                                    %准确率
end
...
% macro_f1(1,n)=0.1*f1_score(1,n)+0.3*sum(f1_score(2:1:size(x,2),n));    %提高故障诊断的得分率
