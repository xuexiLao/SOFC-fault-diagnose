function err = RNN_Bpfun(x, P_train, T_train, P_test, T_test)
%% 训练与测试LSTM网络
%% 输入
% x:一个个体的初始权值和阈值
% P_train:训练样本输入
...
if isempty(recall)
  precision= zeros(4, 30);                                %精确率/查准率
  recall= zeros(4, 30);                                   %召回率/查全率
  f1_score = zeros(4,30);                                 %F1-Score
  macro_f1=zeros(1,30);
  accurate=zeros(1,30);                                   %准确率
end
